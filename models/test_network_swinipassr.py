...
class ResB(nn.Module):
    def __init__(self, channels, group=False):
        super(ResB, self).__init__()
        self.body = nn.Sequential(
            nn.Conv2d(channels, channels, 3, 1, 1, groups=4, bias=True) if group else nn.Conv2d(channels, channels, 3, 1, 1, bias=False),
...
class ParallaxAttentionModule(nn.Module):
    ...
    def __init__(self, channels, layers, group=False):
        ...
        self.rb = ResB(layers * channels, group=group)
...
class SwiniPASSR(nn.Module):
    ...
    def __init__(self, img_size=64, patch_size=1, in_chans=3,
        ...
        self.group = (not (depths[0] % 2))
        self.pam = ParallaxAttentionModule(embed_dim, 1, group=self.group)
    ...
    def forward(self, x_left, x_right):
        ...
        if self.upsampler == 'pixelshuffle':
            ...
            x_left_upscale = torch.nn.functional.interpolate(x_left, scale_factor=self.upscale, 
                                                             ...
                                                             recompute_scale_factor= not self.group)
            ...
            x_leftT, x_rightT, (M_right_to_left, M_left_to_right), (V_left, V_right) = self.pam(catfea_left, catfea_right, x_left, x_right) if self.group else self.pam(x_left, x_right, catfea_left, catfea_right)

            second_catfea_left = self.forward_features(self.fusion(torch.cat([catfea_left, x_leftT], dim=1)), second=True) if self.group else self.forward_features(self.fusion(torch.cat([x_left, x_leftT], dim=1)), second=True)
            ...
            second_catfea_right = self.forward_features(self.fusion(torch.cat([catfea_right, x_rightT], dim=1)), second=True) if self.group else self.forward_features(self.fusion(torch.cat([x_right, x_rightT], dim=1)), second=True)
