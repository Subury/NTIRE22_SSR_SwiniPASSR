...
def main():
    ...
    for idx, path in enumerate(sorted(glob.glob(os.path.join(folder, '*')))):
        ...
        for aumentation_mode in range(8):
            # read image
            imgname, img_lq, img_gt = get_image_pair(args, path, mode=aumentation_mode)  # image to HWC-BGR, float32
        ...
        output = (output * 255.0).round().astype(np.uint8)[:,:int(0.5 * output.shape[1]),:]  # float32 to uint8
        cv2.imwrite(f'{save_dir}/{imgname}.png', output)

        # evaluate psnr/ssim/psnr_b
        if img_gt is not None:
            img_gt = (img_gt * 255.0).round().astype(np.uint8)[:,:int(0.5 * img_gt.shape[1]),:]  # float32 to uint8
...
def get_image_pair(args, path, mode=0):
    ...
    if args.task in ['classical_sr', 'lightweight_sr']:
        ...
        if 'L.png' in path:
            left_img_gt = cv2.imread(path, cv2.IMREAD_COLOR).astype(np.float32) / 255.
            right_img_gt = cv2.imread(path.replace('L.png', 'R.png'), cv2.IMREAD_COLOR).astype(np.float32) / 255.
            ...
            left_img_gt = cv2.imread(path, cv2.IMREAD_COLOR).astype(np.float32) / 255.
            right_img_gt = cv2.imread(path.replace('R.png', 'L.png'), cv2.IMREAD_COLOR).astype(np.float32) / 255.

            left_img_lq = cv2.imread(f'{args.folder_lq}/{imgname}{imgext}', cv2.IMREAD_COLOR).astype(np.float32) / 255.
            right_img_lq = cv2.imread(f'{args.folder_lq}/{imgname}{imgext}'.replace('R.png', 'L.png'), cv2.IMREAD_COLOR).astype(np.float32) / 255.

        img_gt = np.concatenate((left_img_gt, right_img_gt), axis=1)
        img_lq = np.concatenate((left_img_lq, right_img_lq), axis=1)
    ...
    if mode == 0:
        ...
        return imgname, np.flipud(np.rot90(img_lq)), np.flipud(np.rot90(img_gt))
    elif mode == 2:
        return imgname, np.flipud(img_lq), np.flipud(img_gt)
    elif mode == 3:
        return imgname, np.rot90(img_lq, k=3), np.rot90(img_gt, k=3)
    elif mode == 4:
        return imgname, np.flipud(np.rot90(img_lq, k=2)), np.flipud(np.rot90(img_gt, k=2))
    elif mode == 5:
        return imgname, np.rot90(img_lq), np.rot90(img_gt)
    elif mode == 6:
        return imgname, np.rot90(img_lq, k=2), np.rot90(img_gt, k=2)
    elif mode == 7:
        return imgname, np.flipud(np.rot90(img_lq, k=3)), np.flipud(np.rot90(img_gt, k=3))
    # return imgname, img_lq, img_gt
